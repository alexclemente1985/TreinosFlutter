...
class AppDataBase extends _$AppDataBase{ //Rodar dart run build_runner build após instalar as dependências informadas no site do Drift
  ...
  //listinTable já foi instanciado no arquivo gerado
  ...
  Future<List<Listin>> getListins() async {
    ...
    for (ListinTableData row in listData){
      temp.add(Listin(id: row.id.toString(), name: row.name, obs: row.obs, dateCreate: row.dateCreate, dateUpdate: row.dateUpdate));
    }
  ...
  Future<bool> updateListin(Listin listin) async {
    return await update(listinTable)
    .replace(
  ...
  Future<int> deleteListin(int id) async{
    return await (delete(listinTable)
    ..where((row)=> row.id.equals(id))).go(); //go() autoriza o processo do delete | ..where() permite a validação por id
  }
}
...
LazyDatabase _openConnection(){
  return LazyDatabase(() async {
    ...
    if(Platform.isAndroid){
      await applyWorkaroundToOpenSqlite3OnOldAndroidVersions(); //evita problemas com sqlite3 em versões antigas do Android
    }
